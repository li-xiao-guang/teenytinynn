...
class GPT(Layer):
    ...
    def __init__(self, vocabulary_size, context_size, embedding_size, heads):
        ...
        self.transformers = [GPTTransformer(self.context_size, self.embedding_size, self.heads) for _ in range(2)]
        self.output = GPTOutput(self.vocabulary_size, self.embedding_size)

        self.layers = [self.embedding,
                       self.output] + self.transformers
...
class GPTTransformer(Layer):

    def __init__(self, context_size, embedding_size, heads):
        super().__init__()
        self.context_size = context_size
        self.embedding_size = embedding_size
        self.heads = heads
        ...
        self.feed_forwards = [GPTFeedForward(self.embedding_size) for _ in range(2)]

        self.layers = [self.attention] + self.feed_forwards

    def forward(self, x: Tensor):
        x = self.attention(x)
        for l in self.feed_forwards:
            x = l(x)
        return x
...
class GPTAttention(Layer):
    ...
    def forward(self, x: Tensor):
        ...
        query = self.query(x_norm).reshape((-1, self.heads, self.embedding_size))
        key = self.key(x_norm).reshape((-1, self.heads, self.embedding_size))
        value = self.value(x_norm).reshape((-1, self.heads, self.embedding_size))

        scores = self.triu(query @ key.transpose((0, 2, 1)))
        weights = self.softmax(scores)
        vectors = self.merge((weights @ value).reshape((-1, self.heads * self.embedding_size)))
        return x + self.dropout(vectors)
