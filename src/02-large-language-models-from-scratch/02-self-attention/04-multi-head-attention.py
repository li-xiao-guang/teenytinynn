...
query = Linear(feature_embedding.size(), 2 * HEADS, bias_enabled=False)
key = Linear(feature_embedding.size(), 2 * HEADS, bias_enabled=False)
value = Linear(feature_embedding.size(), 2 * HEADS, bias_enabled=False)

query_feature = query(feature_embedding).reshape((-1, HEADS, 2))
key_feature = key(feature_embedding).reshape((-1, HEADS, 2))
value_feature = value(feature_embedding).reshape((-1, HEADS, 2))

attention_scores = query_feature @ key_feature.transpose((0, 2, 1))
print("Attention scores: ", attention_scores.data)

triu = Triu(-np.inf)
attention_scores = triu(attention_scores)

softmax = Softmax(axis=2)
attention_weights = softmax(attention_scores)
dropout = Dropout()
attention_weights = dropout(attention_weights)
print("Attention weights: ", attention_weights.data)
print("Attention weight sum: ", attention_weights.data.sum(axis=2))

context_vectors = (attention_weights @ value_feature).transpose((1, 0, 2))
